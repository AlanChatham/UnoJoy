...
// Since we're the slave, we wait for the master to send
//  the synchronize signal
void slaveSynchronize(void){
  while(digitalRead(SyncPin) == HIGH)
    {}// Hang out
  // Once we get the signal, delay for the timing to work
  delay(100);
}
...
void loop(){
  ...
  // Attempt to synchronize with the other controller
  // It blinks slowly during this time
  blinkAndWait();
