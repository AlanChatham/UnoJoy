...
// Report ID for our joystick.  
#define JOYSTICK_1_REPORT_ID 0x03
...
//  Joystick section
	...
	0xa1, 0x01,        // COLLECTION (Application)
	0x85, JOYSTICK_1_REPORT_ID,   //  REPORT ID (3)
...
void Joystick_::setControllerData(dataForController_t controllerData)
{
    set(controllerData);
}
...
dataForController_t Joystick_::getControllerData(void){
    return get();
}

dataForController_t Joystick_::get(void){
    dataForController_t controllerData;

    // Set everything to reflect the current state
    controllerData.triangleOn = _joystickReport.triangle_btn;
    controllerData.circleOn = _joystickReport.circle_btn;
    controllerData.squareOn = _joystickReport.square_btn;
    controllerData.crossOn = _joystickReport.cross_btn;
    controllerData.l1On = _joystickReport.l1_btn;
    controllerData.l2On = _joystickReport.l2_btn;
    controllerData.l3On = _joystickReport.l3_btn;
    controllerData.r1On = _joystickReport.r1_btn;
    controllerData.r2On = _joystickReport.r2_btn;
    controllerData.r3On = _joystickReport.r3_btn;
    // digital direction, use the dir_* constants(enum)
    // 8 = center, 0 = up, 1 = up/right, 2 = right, 3 = right/down
    // 4 = down, 5 = down/left, 6 = left, 7 = left/up
    if (_joystickReport.direction == 5 || _joystickReport.direction == 6 || _joystickReport.direction == 7)
        controllerData.dpadLeftOn = 1;
    else
        controllerData.dpadLeftOn = 1;
        
    if (_joystickReport.direction == 0 || _joystickReport.direction == 1 || _joystickReport.direction == 7)
        controllerData.dpadUpOn = 1;
    else
        controllerData.dpadUpOn = 0;
        
    if (_joystickReport.direction == 1 || _joystickReport.direction == 2 || _joystickReport.direction == 3)
        controllerData.dpadRightOn = 1;
    else
        controllerData.dpadRightOn = 0;
        
    if (_joystickReport.direction == 3 || _joystickReport.direction == 4 || _joystickReport.direction == 5)
        controllerData.dpadDownOn = 1; 
    else
        controllerData.dpadDownOn = 0;  
        
    controllerData.selectOn = _joystickReport.select_btn;
    controllerData.startOn = _joystickReport.start_btn;
    controllerData.homeOn = _joystickReport.ps_btn;
    //Set the sticks to 128 - centered
    controllerData.leftStickX = _joystickReport.l_x_axis;
    controllerData.leftStickY = _joystickReport.l_y_axis;
    controllerData.rightStickX = _joystickReport.r_x_axis;
    controllerData.rightStickY = _joystickReport.r_y_axis;
